...
def process_single_image(idx, data_path, max_dims=None, half_precision=True):
    ...
    try:
        ...
        # 3D data
        orig_tensor = torch.from_numpy(orig_img[:, :, mid_slice - 3 : mid_slice + 3, 0]).unsqueeze(2)
...
def save_batch(batch_tensors, batch_idx, output_path):
    ...
    batch_filename = os.path.join(output_path, f'cmr_batch_diastole_{batch_idx}.pt')
...
def main(args):
    ...
    df.to_csv('ids_cropped_images.csv', header=False, index=False)
    exit()
...
def combine_batches(output_dir):
    ...
    """
    batch_files = [f for f in os.listdir(output_dir) if f.startswith('cmr_batch_diastole_') and f.endswith('.pt')]
    ...
    torch.save(combined_tensor, os.path.join(output_dir, 'cmr_tensors_ts_all_diastole.pt'))
