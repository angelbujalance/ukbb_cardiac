...
def process_single_image(idx, data_path, max_dims=None, half_precision=True):
    ...
    try:
        ...
        _, _, c, _ = seg_data.shape
        seg_tensor = torch.from_numpy(seg_data[:, :, c // 2, :]).unsqueeze(2)
        ...
        orig_tensor = torch.from_numpy(orig_img[:, :, c // 2, :]).unsqueeze(2)
...
def save_batch(batch_tensors, batch_idx, output_path):
    ...
    batch_filename = os.path.join(output_path, f'cmr_batch_{batch_idx}.pt')
...
def find_max_dimensions(data_list, data_path, sample_size=100):
...
def combine_batches(output_dir):
    ...
    """
    batch_files = [f for f in os.listdir(output_dir) if f.startswith('cmr_batch_') and f.endswith('.pt')]
    ...
    torch.save(combined_tensor, os.path.join(output_dir, 'cmr_tensors_ts_all.pt'))
